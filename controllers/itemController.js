...
exports.showDashboard = async (req, res) => {
  try {
    ...
    const [totalAssistants, totalAssignments, liveAssistant, callLogs] = await Promise.all([
      Assistant.count({ where: { customer_id: customerId } }).then(count => count || 0),
      Assignment.count({ where: { customer_id: customerId } }).then(count => count || 0),
      Assistant.findOne({ where: { customer_id: customerId, status: 1 }, order: [['id', 'DESC']] }).then(assistant => assistant || null),
      CallLog.findAll({ where: { customer_id: customerId }, order: [['id', 'DESC']], limit: 5 })
    ]);

    let liveAssistantAssignmentName = null;
    if (liveAssistant) {
      liveAssistantAssignmentName = await Assignment.findOne({ 
        attributes: ['name'], // Specify the attribute 'name' to retrieve
        where: { id: liveAssistant.assignment } 
      });
      liveAssistantAssignmentName = liveAssistantAssignmentName ? liveAssistantAssignmentName.name : null; // Extract the name attribute
    }
    res.render('dashboard', {totalAssignments, totalAssistants, liveAssistant, callLogs, user: req.user, liveAssistantAssignmentName});
  } catch (err) {
    res.status(500).send(err.message);
  }
};
...
exports.updateAssistant = async (req, res) => {
  ...
  try {
    ...
    const body = {
      name: name,
      firstMessage: String(assignments.first_message),
      voice: {
        voiceId: voice,
        provider: provider
      },
      model: {
        messages: [
          {
            role: 'system',
            content: String(assignments.prompt)
            // content: "You are the friendly and helpful voice of SmartHome Innovations"
          }
        ],
        provider: "openai",
        model: "gpt-3.5-turbo"
...
exports.agentDetails = async (req, res) => {
  try {
    ...
    try {
      ...
      res.render('agent-details', { assistant, assignments, rate, user: req.user });
    } catch (error) {
      console.error('Error fetching assistant details:', error.message);
